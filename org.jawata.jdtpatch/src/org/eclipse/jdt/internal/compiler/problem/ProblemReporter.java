 ...
 *
 * MODIFIED (org.jawata.jdtpatch, 2026-07-11): single change vs the R4_38 tag —
...
public void previewAPIUsed(Scope scope, int sourceStart, int sourceEnd, IBinaryAnnotation previewAnnotation) {
	...
	for (IBinaryElementValuePair valuePair : previewAnnotation.getElementValuePairs()) {
		if (valuePair.getValue() instanceof EnumConstantSignature enumSig) {
			...
			// JAWATA PATCH (org.jawata.jdtpatch, 2026-07-11) for
			// https://github.com/eclipse-jdt/eclipse.jdt.core/issues/5188:
			// getField(..) returns null when the JDK-internal PreviewFeature$Feature
			// enum does not resolve in the current lookup environment (observed under
			// MatchLocator during search) — guard instead of NPE; the feature name
			// stays null and the problem is still reported below.
